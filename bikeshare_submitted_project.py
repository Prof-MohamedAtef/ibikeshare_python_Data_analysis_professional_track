...
def display_raw_data(df):
     """ load just 5 rows according to user input"""
     i = 0
     raw = input("\nWould you like to see 5 rows of the data?\n").lower() # TO DO: convert the user input to lower case using lower() function
...
def station_stats(df):
    ...
    mode= lambda x: x.mode().iat[0] if len(x) > 2 else np.array(x)
    Combination_Station =df.groupby(['Start Station'])['End Station'].apply(pd.Series.mode) 
    
    
    #print('\nMost Commonly used combination of start station and end station trip:', Start_Station, " & ", End_Station)
    print('\nMost Common trip:', Combination_Station )
